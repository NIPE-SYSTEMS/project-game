...
int graphics_main()
{
	for(graphics_fieldpos_x = 0; graphics_fieldpos_x < fieldsize_x; graphics_fieldpos_x++)
	{
		...
		{
			//Creates Field
			graphics_create_field((graphics_fieldpos_x*graphics_offset_x)+graphics_offset_x-spritesize_y, (graphics_fieldpos_y*graphics_offset_y)+graphics_offset_y-spritesize_x, gameplay_field[graphics_fieldpos_x][graphics_fieldpos_y].type);
			
			
			//creates Item
			if(gameplay_field[graphics_fieldpos_x][graphics_fieldpos_y].item != 0)
			{
				graphics_create_field((graphics_fieldpos_x*graphics_offset_x)+graphics_offset_x-spritesize_y, (graphics_fieldpos_y*graphics_offset_y)+graphics_offset_y-spritesize_x, gameplay_field[graphics_fieldpos_x][graphics_fieldpos_y].item);
			}
			
			//creates Fire
			if(gameplay_field[graphics_fieldpos_x][graphics_fieldpos_y].explosion == 1)
			{
			...
			//creates Bomb
			if(gameplay_field[graphics_fieldpos_x][graphics_fieldpos_y].bomb == 1)
			{
	...
	for (i = 0; i < spritesize_y; i++)
	{
		for (b = 0; b < spritesize_x; b++)
		{
				...
				{
					mvaddch((gameplay_player.position_height * graphics_offset_x)+graphics_offset_x-spritesize_y+i, (gameplay_player.position_width * graphics_offset_y)+graphics_offset_y-spritesize_x+b, fields[PLAYER_SPRITE][i][b]);
				}
		}
	}
